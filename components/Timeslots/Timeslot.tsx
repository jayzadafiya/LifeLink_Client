...
import { Timeslots } from "../../interfaces/Doctor";
...
const Timeslot = React.memo(
  ({ timeslots, fees }: TimeslotProps): React.JSX.Element => {
    const sortedTimeslots: Timeslots[] = useMemo(() => {
    ...
    const [newTimeslots, setNewTimeslots] =
      useState<Timeslots[]>(sortedTimeslots);

    const { accessToken } = useSelector((state: RootState) => state.user);

    const openDetails = (time: string, period: string) => {
      setSelectedTime(time);
      setSelectedSlot(period);
      setDialogOpen(true);
    };

    const onChangeDate = (newDateStr: string) => {
    ...
    return (
      <>
        <Head>
          ...
          <meta
            name="description"
            content="Timeslots for appointment booking"
          />
        </Head>
        <div
          className={`${
